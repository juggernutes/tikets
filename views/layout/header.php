...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* contenedor del logo (no empuja el header) */
    .logo-shell{
      width: 144px;
      height: 72px;
      display:flex;
    ...
    .core{
      width: 140px;
      height: 72px;
      background: url("../img/CentroNew.png") no-repeat center/contain;
      border-radius: 999px;
      box-shadow: 
        0 6px 14px rgba(0,0,0,.28);
      transform: translate(-40px, 0px);
      animation: abrirCentro 300ms ease-in-out both;
    }



    /* animación necesaria (la tenías comentada) */
    @keyframes abrirCentro {
      0%   { opacity: 0; transform: translate(-40px, 0px) scale(.2); }
      60%  { opacity: 1; transform: translate(-40px, 0px) scale(1.05); }
      100% { opacity: 1; transform: translate(-40px, 0px) scale(1); }
    }
    ...
    .app-footer__link {
      ...
      font-weight: 700;
      font-size: 13px;
      box-shadow: 0 1px 0 rgba(255, 255, 255, .6) inset, 0 1px 8px rgba(0, 0, 0, .04);
      transition: transform .04s ease, background .15s ease, box-shadow .15s ease;
    }

    .app-footer__link:hover {
      background: #f6fbff;
      box-shadow: 0 6px 18px rgba(15, 98, 146, .18);
    }
